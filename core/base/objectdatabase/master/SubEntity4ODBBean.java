...
import base.entity.master.subentity4.SubEntity4Code;
import base.entity.master.subentity4.SubEntity4ControlId;
import base.entity.master.subentity4.SubEntity4Description;
import base.entity.master.subentity4.SubEntity4Main;
import base.entity.master.subentity4.SubEntity4Note;
...
/**
 * <p>
 * Title: SubEntity4ODBBean
 * </p>
 * <p>
 * Description: SubEntity4 object data.
 * </p>
 * This class implements to persist SubEntity4 information.
 * <P>
 * <p>
 * Converting from internal entity code as long to the string entity code in DTO
 * for easy using.
 * </p>
 * It served as gateway to access all the SubEntity4 tables.
 * <p>
...
// @Stateless
@Stateless(mappedName = "SubEntity4ODBBean/remote")
@Remote({ SubEntityODBRemote.class })
public class SubEntity4ODBBean extends AODBMBase implements SubEntityODBRemote {
	 ...
	 * Code entity.
	 */
	private SubEntity4Code code;
	/**
	 * Main entity.
	 */
	private SubEntity4Main main;
	/**
	 * SubEntity4ControlId entity.
	 */
	private SubEntity4ControlId controlID;
	/**
	...
	/**
	 * Creates a new instance of SubEntity4ODDBean.
	 */
	public SubEntity4ODBBean() {
		super();
		...
		code = new SubEntity4Code();
		main = new SubEntity4Main();
		controlID = new SubEntity4ControlId();
	}

	public long size() {
		logger.fine("SubEntity4Facade:find");

		Query query;
		Object result;
		try {
			query = em.createQuery("SELECT COUNT(c) FROM SubEntity4Code c");
	...
	protected ADescription getNewLingualDescription() {
		return (ADescription) (new SubEntity4Description());
	}

	@Override
	protected ANote getNewNote() {
		return new SubEntity4Note();
	}
	...
	protected DTO assignData(ACode c, AMain m, DTO dto) {
		logger.fine("SubEntity4ODBBean:assignData: " + dto.getCode());
