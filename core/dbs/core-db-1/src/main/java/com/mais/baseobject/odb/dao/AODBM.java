package com.mais.baseobject.odb.dao;

import com.mais.baseobject.odb.Exception.ODBException;
import com.mais.baseobject.odb.dto.Connection;
import com.mais.baseobject.odb.dto.DTO;
import com.mais.baseobject.odb.dto.ShortDTO;
 ...
 * </p>
 * <p>
 * This class implements the default mechanism to add,
...
//    abstract protected AControlId getControlId();
    ...
    /**
     * Return the EntityManager that related to persistence unit of each modules.<BR/><BR/>
     * <B>Using annotation to get the EntityManager.</B><BR/>
     * <Code>@PersistenceContext(unitName="myPersistenceUnit") protected EntityManager entityManager;</Code><BR/>
     * <BR/>or<BR/><BR/>
     * <B>Using EntityManagerFactory to create the EntityManager.</B><BR/>
     * <Code>EntityManagerFactory entityManagerFactory = Persistence.createEntityManagerFactory("default");<BR/>
     * EntityManager entityManager = entityManagerFactory.createEntityManager();</Code><BR/>
...
//        getEntityManager().clear();
        ...
        // The preAdd function will return false, if it does not want to add the data.
...
//        	getEntityManager().getTransaction().rollback();
        ...
        try {
            code = _findCodeInstance(data.getId());
        } catch (Exception e) {
...
//        	context.setRollbackOnly();
        ...
        if (code != null) {
            logger.fine("Duplicate Code: " + data.getId());
//        	context.setRollbackOnly();
            throw new ODBException("Duplicate Code: " + data.getId());
        }

        logger.fine("not Duplicate Code: " + data.getId());

// TODO coding here ***************************

//        Check authorization to add the data
//        Should to search main from id.
//        if (Pattern.matches(main.getAllowUserEdit(), connectionInfo.getUserId()))
...
// TODO implement replicate add data here.
        ...
        try {
            ...
            logger.warning("Could not add: " + data.getCode());
        	context.setRollbackOnly();
            ex.printStackTrace();
            throw new ODBException("Could not add: " + data.getCode() + "/n" + ex);
        }
...
//			publishPersistence.add(pInfo, connectionInfo, data);
//			publishPersistence.add(this.getClass().getName(), connectionInfo, data);
        	  }
        }
...
//            pInfo.setReplicate(data.getReplicate());
//            pInfo.setReplicateTo(data.getReplicateSite());
//			publishPersistance.add(pInfo, currentInfo, data);
      	  if (data.getReplicate()) {
//	            PublishPersistence publishPersistence = new PublishPersistence();
...
////		}
     ...
     * @param id string to be search
     * @return ACode entity subclass instance
     */
//    @TransactionAttribute(TransactionAttributeType.SUPPORTS)
    protected ACode _findCodeInstance(String id) {
        logger.fine("_findCodeInstance: id = " + id);

        if (code == null)
        		code = getCodeEntity();

        String SQLquery = "SELECT c FROM ";
        SQLquery = SQLquery + getCodeEntity().getClass().getSimpleName();
        SQLquery = SQLquery + " c WHERE c.id = :id";
        logger.fine("Statement: " + QLquery);
        try {
//            Query q = em.createQuery(SQLquery);
            Query q = getEntityManager().createQuery(SQLquery);
            code = (ACode) q.setParameter("id", id).getSingleResult();
        } catch (Exception e) {
//        	getEntityManager().clear();
            /**
             * No exception stack trace required
